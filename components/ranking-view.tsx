  ...
  return (
    <span
      className={`inline-flex items-center overflow-hidden rounded-md border border-[color:var(--border-subtle)] bg-[color:var(--surface-base)] text-xs ${
        tone === "accent" ? "text-[color:var(--accent-strong)]" : "text-[color:var(--text-muted)]"
      }`}
    >
      <span className="px-2 py-1 font-medium normal-case tracking-normal">
        {label}
      </span>
                    ...
                    {leaderboard.map((entry) => {
                      ...
                      return (
                        <TableRow key={entry.userId}>
                          ...
                          <TableCell className="text-center text-md font-bold">
                            {entry.totalPoints}
                          </TableCell>
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-2">
                      {rule?.enableMatchPredictions ? (
                        <>
                          <RulePill label="Exato" points={rule.scoring.exactScore} />
                          <RulePill label="Resultado" points={rule.scoring.correctOutcome} />
                        </>
                      ) : null}

                      {rule?.enablePlacementPredictions ? (
                        <>
                          <RulePill label="Campeão" points={rule.scoring.champion} />
                          <RulePill label="Vice" points={rule.scoring.runnerUp} />
                          <RulePill label="3º" points={rule.scoring.thirdPlace} />
                        </>
                      ) : null}
                    </div>
                  </div>
